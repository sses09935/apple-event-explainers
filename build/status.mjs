...
export function inspectStatus(root=ROOT){
 ...
 try{
  ...
  Object.assign(hosting,{profile:plan.profile,local_preflight:plan.ready?'passed':'blocked',blockers:plan.errors,engineering_verified:verification.status==='current'&&verification.gate===plan.verification_gate&&verification.profile===plan.profile,required_verification_gate:plan.verification_gate,content_certification_required:!policy.engineeringOnly});
 ...
 return {
  ...
  limits:['Local snapshot only; saved verification must match current input, public source and tested output.','Hosting local_preflight checks the configured live target and current artifact without saving a deployment plan, authorizing a write or querying the remote environment.','Draft Hosting readiness is separate from release/production content certification; unfinished coverage and semantic review remain reported.','Preflight is not full engineering verification or semantic approval.','Historical delivery and deployment receipts are not used to infer current remote state.']
 };
}
